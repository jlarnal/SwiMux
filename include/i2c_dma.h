...
typedef void (*DmaWriteCompleteCb)(void);

// I2C Parameters
#define I2C_CLKRATE 1000000 // I2C bus clock rate (Hz)
...
// I2C Functions
void I2C_init(uint32_t clockRate); // I2C init function
void I2C_start(uint8_t addr, int restart); // I2C start transmission, addr must contain R/W bit
...
void I2C_writeBuffer(uint8_t* buf, uint16_t len, int stopOnCompletion);
void I2C_readBuffer(uint8_t* buf, uint16_t len);
void I2C_setUserCallback(DmaWriteCompleteCb completionCallback);

#define I2C_sendBuffer(addr, buf, len, restart, stopOnCompletion)                                                                                             \
    {                                                                                                                                                \
        I2C_start(addr, restart);                                                                                                                             \
        I2C_writeBuffer(buf, len, stopOnCompletion);                                                                                                 \
    }

#define I2C_getBuffer(addr, buf, len, stop)                                                                                                          \
    {                                                                                                                                                \
